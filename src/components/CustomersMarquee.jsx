...
function MarqueeRow({ items, duration = 40 }) {
  // Duplicate once so translating -50% loops cleanly
  const track = [...items, ...items]

  return (
    // IMPORTANT: lock the marquee to LTR so RTL doesn’t affect animation math.
    <div className="marquee relative overflow-hidden mask-fade-x w-full" dir="ltr" style={{direction:'ltr'}}>
      <div
        className="marquee-track inline-flex items-center"
        style={{ animationDuration: `${duration}s`, gap: '5rem' }}
      >
        {track.map((logo, i) => (
          <div
            key={`${logo.name}-${i}`}
  ...
  return (
    <section className="wrap py-16">
      ...
      <div className="mt-8">
        ...
        {/*
        Optional counter-row (kept LTR as well):
        <div className="mt-6">
